    ...
    <div className="w-full min-h-screen bg-primary">
      <div className="container h-full flex flex-col gap-10 my-6">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl md:text-3xl font-medium text-secondary">Your Networks</h1>
          <AddNetworkModal />
        </div>
        ...
        {status === "pending" ?
          <div></div>
          : status === "success" ?
            <div className="grid grid-cols-2 lg:grid-cols-6 sm:grid-cols-4 gap-6">
              {networks && networks.map((n) =>
                <Link to={`/network/${n.id}`}>
                  <div className={cn("w-full h-full p-6 flex flex-col gap-4 border rounded-2xl border-l-4 border-b-4")}>
                    <h3 className="text-xl md:text-2xl font-semibold text-secondary">{n.name}</h3>
